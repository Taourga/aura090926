...
function TaskButton({ task, today, technical }: { task: CleaningTask; today: boolean; technical: boolean }) {
  const [pending, startTransition] = useTransition();
  const [result, setResult] = useState<{ error?: string; success?: string }>({});
  const router = useRouter();
  return <div>
    {task.completed_at ? <span className="cleaning-done">✓ {parisDateTime(task.completed_at)}<small>{task.completed_name}</small></span> : technical && today ? <button className="button button-primary button-small" disabled={pending} onClick={() => startTransition(async () => { try { setResult(await completeCleaning(task.id)); router.refresh(); } catch { setResult({ error: "Le pointage n’a pas pu être confirmé. Actualisez puis réessayez." }); } })}>{pending ? "Validation…" : "Valider le nettoyage"}</button> : <span className="badge badge-warning">À faire</span>}
...
export function HousekeepingDashboard({ data, date, technical }: { data: HousekeepingData; date: string; technical: boolean }) {
  const router = useRouter();
  const today = date === parisDate();
  ...
  return <>
    ...
    {!today && <p className="notice">{date > parisDate() ? "Les tâches de ménage seront générées le jour concerné. Vous pouvez préparer les affectations et consulter les prévisions de repas." : "Historique des pointages enregistrés. Les jours antérieurs à l’activation du module peuvent ne contenir aucun pointage."}</p>}
    ...
    <p className="row-meta">100 chambres : 001–010, 101–130, 201–230, 301–330. Le jour d’un retour après au moins 24 h d’absence, la chambre est retirée de la liste sauf entrée ou sortie définitive prévue. Pointage midi à partir de 12 h et soir à partir de 18 h.</p>
