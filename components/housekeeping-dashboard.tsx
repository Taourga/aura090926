...
export function HousekeepingDashboard({ data, date, technical, timezone, locale }: { data: HousekeepingData; date: string; technical: boolean; timezone: string; locale: string }) {
  ...
  return <>
    <div className="page-intro"><div><div className="section-kicker">{technical ? "Feuille de service" : "Pilotage hôtelier"}</div><h1>{technical ? "Mes nettoyages" : "Gouvernance hôtelière"}</h1><p>{technical ? "Vos zones à traiter aujourd’hui, avec validation immédiate une fois le nettoyage terminé." : "Chambres, équipe, suivi du ménage et prévisions de repas sur un seul écran."}</p></div><div className="housekeeping-head-actions"><form className="cleaning-date" action="/portal/housekeeping"><label>Date<input type="date" name="date" required defaultValue={date} /></label><button className="button button-secondary">Consulter</button><button type="button" className="button button-secondary" onClick={() => router.refresh()}>Actualiser</button></form><div className="page-intro-actions"><Link className="button button-secondary button-small" href="/portal/pulse">AURA Pulse</Link>{!technical && <Link className="button button-secondary button-small" href="/portal/discharges">Sorties prévues</Link>}</div></div></div>
