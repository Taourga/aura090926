...
export function PermissionForm({ minNoticeHours }: { minNoticeHours: number }) {
  ...
  const minDeparture = new Date(Date.now() + minNoticeHours * 60 * 60 * 1000).toISOString().slice(0, 16);
  return <form onSubmit={onSubmit}>
    ...
    <p className="form-help">Une demande doit être envoyée au moins {minNoticeHours} heures avant le début souhaité de la permission.</p>
