...
export function AppointmentForm({ patients }: { patients: { id: string; full_name: string }[] }) {
  ...
  return <form onSubmit={onSubmit}><ActionFeedback message={result.success} error={result.error} /><div className="form-grid"><label className="field">Patient<select name="patientId" defaultValue="" required><option value="" disabled>Choisir un patient</option>{patients.map((patient) => <option value={patient.id} key={patient.id}>{patient.full_name}</option>)}</select></label><label className="field">Intitulé<input name="title" required placeholder="Ex. entretien individuel" /></label><label className="field">Début<input name="startsAt" type="datetime-local" required /></label><label className="field">Fin<input name="endsAt" type="datetime-local" required /></label><label className="field">Lieu<input name="location" placeholder="Salle ou unité" /></label><label className="field">Notes (facultatif)<input name="notes" placeholder="Information visible au patient" /></label></div><button className="button button-primary" disabled={loading}>{loading ? "Ajout..." : "Ajouter au planning"}</button></form>;
}
