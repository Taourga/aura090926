...
begin
  ...
  if not exists(select 1 from public.trusted_contacts tc where tc.facility_id=v_facility and tc.patient_id=p_patient_id) then
    raise exception 'Aucune personne de confiance renseignée pour ce patient';
  end if;
  ...
  perform public.write_audit(
    ...
    'trusted_contact',
    p_patient_id,
    jsonb_build_object('enabled',p_enabled,'scopes',v_scopes)
  );
end $$;
