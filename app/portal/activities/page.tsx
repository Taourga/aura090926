...
export default async function ActivitiesPage({ searchParams }: { searchParams: Promise<{ section?: string }> }) {
  ...
  const [{ data: activities }, { data: enrollments }, { data: enrollmentCounts }, { data: sportData }, { data: facilitatorRows }, { data: directoryRows }, { data: updateRows }] = await Promise.all([
  ...
  const countByActivity = new Map(((enrollmentCounts || []) as { activity_id: string; enrolled_count: number | string }[]).map((item) => [item.activity_id, Number(item.enrolled_count)]));
  ...
  const directory = new Map(((directoryRows || []) as Directory[]).map((item) => [item.id, item]));
  ...
  const canMarkAttendance = ["doctor", "manager", "psychologist", "provider", "governance", "coach", "nurse", "admin"].includes(profile.role);

  return <PortalShell profile={profile}>
    <div className="page-intro"><div><h1>Activités</h1><p>Choisissez un atelier et sachez toujours qui vous accompagne.</p></div></div>
    ...
    {showSport ? <div className={canManageSport ? "dashboard-grid" : "stack"}>
      ...
      {canManageSport && <aside className="card"><div className="card-header"><div><h2>Modifier le planning</h2></div></div><div className="card-body"><SportRoomScheduleForm /></div></aside>}
    </div> : <div className={canManageActivities ? "dashboard-grid" : "stack"}>
      <section className="card"><div className="card-header"><div><h2>Prochaines activités</h2><p className="card-subtitle">Chaque activité affiche son intervenant et ses éventuelles modifications.</p></div></div><div className="card-body"><div className="list">{activities?.length ? activities.map((activity) => {
        const used = countByActivity.get(activity.id) || 0;
        const enrolled = enrollmentList.find((item) => item.activity_id === activity.id && item.patient_id === profile.id);
        const attendees = enrollmentList.filter((item) => item.activity_id === activity.id);
        const link = facilitators.find((item) => item.activity_id === activity.id && item.is_primary) || facilitators.find((item) => item.activity_id === activity.id);
        const facilitator = link ? directory.get(link.clinician_id) : undefined;
        const latest = updates.find((item) => item.activity_id === activity.id);
        const canPublish = !!facilitator?.user_id && facilitator.user_id === profile.id || canManageActivities;
        return <div key={activity.id} className="stack">
          <div className="list-row" style={{ gridTemplateColumns: "110px minmax(0,1fr) auto" }}><div className="time">{formatDateTime(activity.starts_at)}</div><div><div className="row-title">{activity.title}</div><div className="row-meta">{activity.description || ""}{activity.location ? ` · ${activity.location}` : ""} · {used}/{activity.capacity} places</div><div className="activity-facilitator"><span className="patient-avatar" aria-hidden="true">{facilitator?.full_name.split(" ").map((part) => part[0]).slice(0,2).join("") || "?"}</span><span><strong>{facilitator?.full_name || "Intervenant à confirmer"}</strong><small>{facilitator?.specialty || "Intervenant de l’activité"}</small></span></div>{latest && <div className={`activity-alert activity-alert--${latest.update_type}`}><strong>{latest.update_type === "absence" ? "Intervenant absent" : latest.update_type === "change" ? "Modification" : "Information"}</strong><span>{latest.message}</span></div>}</div>{profile.role === "patient" ? <div><ActivityButton activityId={activity.id} enrolled={Boolean(enrolled)} full={used >= activity.capacity} />{enrolled && <span className="badge badge-info">{attendanceLabels[enrolled.attendance_status as AttendanceStatus]}</span>}</div> : <span className="badge badge-info">{used} inscrit{used > 1 ? "s" : ""}</span>}</div>
          {canPublish && <div className="activity-publish-box"><strong>Informer les patients inscrits</strong><ActivityUpdateForm activityId={activity.id} /></div>}
          {canMarkAttendance && attendees.length > 0 && <div className="attendance-list"><strong>Présences des inscrits</strong>{attendees.map((attendee) => { const patient = Array.isArray(attendee.patient) ? attendee.patient[0] : attendee.patient; return <div className="list-row" key={attendee.id} style={{ gridTemplateColumns: "minmax(0,1fr) auto auto" }}><span>{patient?.full_name || "Patient"}</span><span className="badge badge-info">{attendanceLabels[attendee.attendance_status as AttendanceStatus]}</span><AttendanceActions kind="activity" recordId={attendee.id} /></div>; })}</div>}
        </div>;
      }) : <p className="empty">Aucune activité n&apos;est programmée pour le moment.</p>}</div></div></section>
      {canManageActivities && <aside className="card"><div className="card-header"><div><h2>Ajouter une activité</h2></div></div><div className="card-body"><ActivityForm /></div></aside>}
    </div>}
  </PortalShell>;
}
