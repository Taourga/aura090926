...
export default async function HandoffPage() {
  ...
  const [permissionsQ, visitsQ, dischargesQ, tasksQ, activityUpdatesQ, doctorAbsencesQ, stayRefsQ] = await Promise.all([
    supabase.from("permission_requests").select("id,patient_id,status,departure_at,return_at,patient:profiles!permission_requests_patient_id_fkey(full_name)").in("status", ["waiting", "submitted", "approved", "departed"]).order("departure_at").limit(40),
    supabase.from("visit_notifications").select("id,patient_id,status,scheduled_start,scheduled_end,visitor_one_name,patient:profiles!visit_notifications_patient_id_fkey(full_name)").gte("scheduled_start", now.toISOString()).lte("scheduled_start", in24h.toISOString()).order("scheduled_start").limit(30),
    supabase.rpc("discharge_planning_board"),
    supabase.from("operational_tasks").select("id,patient_id,title,assigned_role,due_at,status,patient:profiles!operational_tasks_patient_id_fkey(full_name)").eq("status", "pending").eq("assigned_role", "nurse").order("due_at", { ascending: true, nullsFirst: false }).limit(30),
  ...
  const stayPatientMap = new Map((stayRefsQ.data || []).map((stay) => [stay.id, stay.patient_id]));
  const discharges = ((dischargesQ.data || []) as { stay_id: string; patient_name: string; room_number: string | null; planned_discharge_at: string | null }[])
    ...
    .map((d) => ({ ...d, patient_id: stayPatientMap.get(d.stay_id) || null }));
  ...
  return <PortalShell profile={profile}>
    ...
    <section className="handoff-grid">
      <article className="work-card work-card--priority"><div className="work-card-head"><div><p className="section-kicker">Priorités</p><h2>À connaître maintenant</h2></div><span className="count-pill">{lateReturns.length + waiting.length}</span></div><div className="work-card-body">{lateReturns.map((p) => { const patient = one(p.patient as Rel); return <div className="record-row" key={`late-${p.id}`}><div><strong>Retour en retard · {patient?.full_name || "Patient"}</strong><small>Retour attendu à {tm(p.return_at)}</small></div><Link className="text-link" href={`/portal/permissions?patient=${p.patient_id}`}>Voir →</Link></div>; })}{waiting.slice(0, 5).map((p) => { const patient = one(p.patient as Rel); return <div className="record-row" key={`wait-${p.id}`}><div><strong>Permission en attente · {patient?.full_name || "Patient"}</strong><small>Départ souhaité {dt(p.departure_at)}</small></div><Link className="text-link" href={`/portal/permissions?patient=${p.patient_id}`}>Voir →</Link></div>; })}{!lateReturns.length && !waiting.length && <p className="empty">Aucun point critique.</p>}</div></article>
      <article className="work-card"><div className="work-card-head"><div><p className="section-kicker">Sorties</p><h2>À préparer</h2></div><Link className="text-link" href="/portal/discharges">Planning →</Link></div><div className="work-card-body">{discharges.length ? discharges.map((d) => <div className="record-row" key={d.stay_id}><div><strong>{d.patient_name}</strong><small>Chambre {d.room_number || "—"} · {dt(d.planned_discharge_at)}</small></div>{d.patient_id ? <Link className="text-link" href={`/portal/discharges?patient=${d.patient_id}`}>Ouvrir →</Link> : <span className="badge badge-warning">À préparer</span>}</div>) : <p className="empty">Aucune sortie dans les prochaines 24 h.</p>}</div></article>
      <article className="work-card"><div className="work-card-head"><div><p className="section-kicker">Mes tâches</p><h2>Coordination de sortie</h2></div><span className="count-pill">{tasks.length}</span></div><div className="work-card-body">{tasks.length ? tasks.slice(0, 8).map((task) => { const patient = one(task.patient as Rel); return <div className="record-row" key={task.id}><div><strong>{task.title}</strong><small>{patient?.full_name || "Patient"} · {task.due_at ? dt(task.due_at) : "Sans échéance"}</small>{task.patient_id && <Link className="text-link" href={`/portal/discharges?patient=${task.patient_id}`}>Voir la sortie →</Link>}</div><OperationalTaskButton taskId={task.id} /></div>; }) : <p className="empty">Aucune tâche infirmière en attente.</p>}</div></article>
