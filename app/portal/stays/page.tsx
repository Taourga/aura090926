...
export default async function StaysPage() {
  ...
  if (!["reception", "nurse", "admin"].includes(profile.role)) redirect("/portal");
  ...
  if (patients.error || stays.error || admissions.error) return <PortalShell profile={profile}><h1>Entrées & sorties</h1><p role="alert">Chargement impossible. Réessayez ou contactez l’administrateur pour vérifier l’activation du module.</p></PortalShell>;
  const name = (id: string) => patients.data.find(p => p.id === id)?.full_name || "Patient";
  const canAdmit = profile.role !== "nurse";
  const canDischarge = profile.role !== "reception";
  ...
  return <PortalShell profile={profile}><div className="page-intro"><div><h1>Entrées & sorties définitives</h1><p>L’accueil renseigne les premières entrées ; les infirmiers renseignent les sorties définitives. Les prévisions alimentent l’hôtellerie et les repas.</p></div></div>
    ...
    <section className="card"><div className="card-header"><h2>Séjours et sorties définitives</h2></div><div className="card-body data-table-wrap"><table className="data-table"><thead><tr><th>Patient</th><th>Chambre</th><th>Entrée réelle</th><th>Sortie définitive · heure de Paris</th></tr></thead><tbody>{stays.data.map(s => <tr key={s.id}><td>{name(s.patient_id)}</td><td>{s.room_number || "Non affectée"}</td><td>{parisDateTime(s.started_at)}</td><td>{canDischarge ? <DischargeForm key={`${s.id}-${s.planned_discharge_at}`} id={s.id} expected={s.planned_discharge_at} /> : s.planned_discharge_at ? parisDateTime(s.planned_discharge_at) : "Non renseignée"}</td></tr>)}{!stays.data.length && <tr><td colSpan={4} className="empty">Aucun séjour actif.</td></tr>}</tbody></table></div></section>
  </PortalShell>;
}
