...
export default async function VisitsPage() {
  ...
  const renderRow = (visit: NonNullable<typeof visits>[number]) => { const patient=Array.isArray(visit.patient)?visit.patient[0]:visit.patient; const visitors=[visit.visitor_one_name,visit.visitor_two_name].filter(Boolean).join(" · "); const action=profile.role==="reception"&&(visit.status==="scheduled"||visit.status==="arrived")?(visit.status==="scheduled"?"arrive":"depart"):null; return <div className="list-row" key={visit.id}><div className="time">{displayDateTime(visit.scheduled_start)}</div><div><div className="row-title">{profile.role==="reception"?patient?.full_name||"Patient":"Visite prévue"}</div><div className="row-meta">{visitors} · jusqu&apos;au {displayDateTime(visit.scheduled_end)}</div></div><span className="badge badge-info">{visitLabels[visit.status as VisitStatus]}</span>{action&&<VisitMovementActions visitId={visit.id} action={action}/>}</div>; };
  return <PortalShell profile={profile}>
    <div className="page-intro"><div><div className="section-kicker">Visiteurs</div><h1>{profile.role==="patient"?"Mes visites":"Visiteurs attendus"}</h1><p>{profile.role==="patient"?"Choisissez un jour, renseignez votre visiteur et l’accueil est prévenu.":"Les arrivées et départs se traitent en un clic."}</p></div></div>
    ...
    <section className="card"><div className="card-header"><div><h2>{profile.role==="patient"?"Mes prochaines visites":"À accueillir"}</h2><p className="card-subtitle">Seulement les informations utiles.</p></div></div><div className="card-body"><div className="list">{futureVisits.length?futureVisits.map(renderRow):<p className="empty">Aucune visite à venir.</p>}</div></div></section>
