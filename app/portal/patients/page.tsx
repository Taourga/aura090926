...
export default async function PatientsPage({ searchParams }: { searchParams: Promise<{ patient?: string; filter?: string }> }) {
  ...
  const [{ data: doctors }, { data: rosterRows }, { data: patients }, { data: stays }, { data: contacts }, { data: trustedRows }, { data: activePermissionRows }] = await Promise.all([
  ...
  const basePatients = ((patients || []) as Patient[]).filter((p)=>myIds.has(p.id));
  ...
  const patientList = basePatients.filter(patient=>{
    const stay=stayByPatient.get(patient.id); const permission=permissionByPatient.get(patient.id);
    if(filter==="present") return stay?.presence!=="out";
    if(filter==="out") return stay?.presence==="out" || permission?.status==="departed";
    if(filter==="permission") return !!permission && ["submitted","waiting"].includes(permission.status);
    if(filter==="approved") return !!permission && permission.status==="approved";
    return true;
  });
  ...
  const [{ data: permissions }, { data: appointments }, { data: enrollments }] = selected ? await Promise.all([
    ...
    supabase.from("activity_enrollments").select("id,activity:activities(title,starts_at,location,requires_prescription)").eq("patient_id",selected.id).limit(8),
  ]) : [{data:[]},{data:[]},{data:[]}];
  ...
  const prescribedActivities = enrollmentList.filter(item=>one(item.activity)?.requires_prescription);
  const otherActivities = enrollmentList.filter(item=>!one(item.activity)?.requires_prescription);
  ...
  const presentCount=basePatients.filter(p=>stayByPatient.get(p.id)?.presence!=="out").length;
  const outCount=basePatients.filter(p=>stayByPatient.get(p.id)?.presence==="out").length;
  const pendingCount=basePatients.filter(p=>["submitted","waiting"].includes(permissionByPatient.get(p.id)?.status||"")).length;
  const approvedCount=basePatients.filter(p=>permissionByPatient.get(p.id)?.status==="approved").length;
  ...
  return <PortalShell profile={profile}>
    ...
    <div className="doctor-simple-layout">
      <aside className="doctor-simple-list">
        {patientList.map((patient)=>{ const s=stayByPatient.get(patient.id); const p=permissionByPatient.get(patient.id); return <Link key={patient.id} href={`/portal/patients?filter=${filter}&patient=${patient.id}`} className={`doctor-simple-patient${selected?.id===patient.id?" active":""}`}><span>{initials(patient.full_name)}</span><div><strong>{patient.full_name}</strong><small>Ch. {s?.room_number || "—"} · {s?.presence === "out" ? "En permission" : p&&["submitted","waiting"].includes(p.status)?"Permission à valider":"Présent"}</small></div></Link>; })}
      ...
      <section className="doctor-simple-record">
        {selected ? <>
          <div className="doctor-simple-header">
            ...
            <div className="doctor-simple-actions"><Link className="button button-secondary button-small" href={`/portal/appointments?patient=${selected.id}`}>◷ Planning</Link></div>
          </div>
          ...
          <div className="doctor-simple-grid">
            <article className="doctor-simple-card"><div className="doctor-simple-card-head"><h3>Coordonnées</h3></div>{patientPhone ? <a className="contact-link" href={`tel:${patientPhone.replace(/\s+/g,"")}`}>{patientPhone}</a> : <strong>Téléphone non renseigné</strong>}{patientEmail ? <a className="contact-link" href={`mailto:${patientEmail}`}>{patientEmail}</a> : <span>Email non renseigné</span>}<span>{[contact?.address_line1,[contact?.postal_code,contact?.city].filter(Boolean).join(" ")].filter(Boolean).join(" · ") || "Adresse non renseignée"}</span></article>

            <article className="doctor-simple-card doctor-simple-card--trusted"><div className="doctor-simple-card-head"><h3>Personne de confiance</h3>{trusted && <span className={activePortal?"badge badge-success":"badge badge-neutral"}>{activePortal?"Portail actif":"Contact uniquement"}</span>}</div>{trusted ? <><strong>{trusted.full_name} · {trusted.relationship}</strong>{trusted.phone ? <a className="contact-link" href={`tel:${trusted.phone.replace(/\s+/g,"")}`}>☎ {trusted.phone}</a> : <span>Téléphone non renseigné</span>}{trusted.email ? <a className="button button-secondary button-small" href={`mailto:${trusted.email}`}>✉ Envoyer un email</a> : <span>Email non renseigné</span>}{trusted.is_emergency_contact && <small>Contact d’urgence</small>}</> : <span>Non renseignée</span>}</article>

            <article className="doctor-simple-card"><div className="doctor-simple-card-head"><h3>Prochainement</h3><Link href={`/portal/appointments?patient=${selected.id}`}>Voir tout →</Link></div>{appointmentList.length ? appointmentList.slice(0,3).map((a)=><div className="doctor-simple-row" key={a.id}><strong>{a.title}</strong><span>{formatDateTime(a.starts_at)} · {a.location || "Lieu à confirmer"}</span></div>) : <span>Aucun rendez-vous à venir.</span>}</article>

            <article className="doctor-simple-card"><div className="doctor-simple-card-head"><h3>Permissions</h3><Link href={`/portal/permissions?patient=${selected.id}`}>Traiter →</Link></div>{permissionList.length ? permissionList.slice(0,3).map((p)=><div className="doctor-simple-row" key={p.id}><div><strong>{formatDateTime(p.departure_at)}</strong><span>Retour {formatDateTime(p.return_at)}</span></div><StatusBadge status={p.status as PermissionStatus} /></div>) : <span>Aucune permission.</span>}</article>

            <article className="doctor-simple-card doctor-simple-card--prescribed"><div className="doctor-simple-card-head"><h3>Activités prescrites</h3><span className="badge badge-info">{prescribedActivities.length}</span></div>{prescribedActivities.length?prescribedActivities.map(e=>{const a=one(e.activity);return <div className="doctor-simple-row" key={e.id}><strong>{a?.title||"Activité"}</strong><span>{a?formatDateTime(a.starts_at):""} · {a?.location||"Lieu à confirmer"}</span></div>}):<span>Aucune activité prescrite.</span>}</article>
          </div>

          <details className="doctor-simple-more"><summary>Plus d’informations</summary><div className="doctor-simple-more-grid"><div><h3>Autres activités</h3>{otherActivities.map((e)=>{ const a=one(e.activity); return <p key={e.id}><strong>{a?.title || "Activité"}</strong><br/><span>{a ? formatDateTime(a.starts_at) : ""}</span></p>; })}{!otherActivities.length && <p>Aucune autre activité.</p>}</div>{trusted && <div><h3>Partage avec le proche</h3><TrustedContactConsent patientId={selected.id} trustedName={trusted.full_name} initialEnabled={activePortal} initialScopes={trusted.scopes} initialNotifications={trusted.notification_preferences} initialExpiresAt={trusted.access_expires_at} initialEmergencyContact={trusted.is_emergency_contact} initialPreferredContactMethod={trusted.preferred_contact_method} canManage hasPortalAccount={!!trusted.user_id} /></div>}</div></details>
