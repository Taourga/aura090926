...
export default async function PermissionsPage({ searchParams }: { searchParams: Promise<{ patient?: string }> }) {
  ...
  const subtitle = profile.role === "patient" ? "Votre calendrier d’abord : cliquez sur un jour pour demander, consulter, modifier ou annuler une permission." : contextualPatient ? "Vue filtrée sur le patient sélectionné : décision et historique restent au même endroit." : profile.role === "reception" ? "Enregistrez l’heure réelle au moment du départ et du retour." : "Consultez et traitez les demandes selon votre périmètre.";

  return <PortalShell profile={profile}>
    <div className="page-intro"><div><div className="section-kicker">Sorties temporaires</div><h1>{title}</h1><p>{subtitle}</p></div>{contextualPatient && profile.role === "doctor" && <div className="page-intro-actions"><Link className="button button-secondary" href={`/portal/patients?patient=${contextualPatient.id}`}>← Fiche patient</Link><Link className="button button-primary" href={`/portal/messages?contact=${contextualPatient.id}`}>Message</Link></div>}</div>
