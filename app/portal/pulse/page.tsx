...
type Permission = { id: string; departure_at: string; return_at: string; status: string; doctor_decision: string | null; manager_decision: string | null; patient: Rel };
...
type UnreadMessage = { id: string; body: string; priority: number; created_at: string; sender: Rel };
...
export default async function PulsePage() {
  ...
  const [staysQ, permsQ, apptsQ, visitsQ, dischargesQ] = await Promise.all([
    ...
    supabase.from("permission_requests").select("id,departure_at,return_at,status,doctor_decision,manager_decision,patient:profiles!permission_requests_patient_id_fkey(full_name)").in("status", ["submitted", "waiting", "approved", "departed"]).order("departure_at").limit(12),
  ...
  if (canMessage) {
    const [contactsQ, unreadQ] = await Promise.all([
      ...
      supabase.from("clinical_messages").select("id,body,priority,created_at,sender:profiles!clinical_messages_sender_id_fkey(full_name)").eq("recipient_id", profile.id).is("read_at", null).order("priority", { ascending: false }).order("created_at", { ascending: false }).limit(5),
    ]);
  ...
  return <PortalShell profile={profile}>
    ...
    <section className="pulse-metrics">
      ...
      <Link href="/portal/appointments" className="pulse-metric"><b>◷</b><div><strong>{appointments.length}</strong><span>Rendez-vous à venir</span><small>Prochaines 36 h</small></div></Link>
      <Link href={showVisits ? "/portal/visits" : "/portal"} className="pulse-metric"><b>♧</b><div><strong>{onSite.length}</strong><span>Visiteurs sur site</span><small>{visits.length} aujourd’hui</small></div></Link>
    ...
    <section className="pulse-grid">
      <article className="pulse-card"><header><div><h2>◎ Patients</h2><p>{stays.length} séjours actifs</p></div><Link href="/portal/discharges">Préparer les sorties →</Link></header><div>{stays.slice(0, 6).map((s) => <Link className="pulse-row pulse-row-link" href="/portal/discharges" key={s.id}><div><strong>{nameOf(s.patient)}</strong><small>Chambre {s.room_number || "—"}</small></div><em>{presence(s.presence)}</em></Link>)}</div></article>
      ...
      <article className="pulse-card"><header><div><h2>✓ Permissions</h2><p>{permissions.length} demandes actives</p></div><Link href="/portal/permissions">Voir toutes →</Link></header><div>{permissions.slice(0, 6).map((p) => {
        ...
        return <div className="pulse-action-row" key={p.id}><div className="pulse-row"><div><strong>{nameOf(p.patient)}</strong><small>{dt(p.departure_at)}</small></div><em>{permissionLabel(p.status)}</em></div>{canReview ? <PermissionDecisionActions permissionId={p.id} /> : movement ? <MovementActions permissionId={p.id} action={movement} /> : <Link className="pulse-inline-link" href="/portal/permissions">Ouvrir →</Link>}</div>;
      })}</div></article>

      <article className="pulse-card"><header><div><h2>◷ Rendez-vous</h2><p>{appointments.length} prochains créneaux</p></div><Link href="/portal/appointments">Planning →</Link></header><div>{appointments.slice(0, 6).map((a) => <Link href="/portal/appointments" className="pulse-time pulse-row-link" key={a.id}><time>{tm(a.starts_at)}</time><div><strong>{nameOf(a.patient)}</strong><small>{a.title}{a.location ? ` · ${a.location}` : ""}</small></div></Link>)}</div></article>

      <article className="pulse-card"><header><div><h2>♧ Visites</h2><p>{visits.length} aujourd’hui</p></div>{showVisits && <Link href="/portal/visits">Voir toutes →</Link>}</header><div>{visits.length ? visits.slice(0, 6).map((v) => <div className="pulse-action-row" key={v.id}><div className="pulse-row"><div><strong>{v.visitor_one_name}</strong><small>{nameOf(v.patient)} · {tm(v.scheduled_start)}</small></div><em>{v.status === "arrived" ? "Sur site" : v.status === "departed" ? "Terminée" : "À venir"}</em></div>{profile.role === "reception" && (v.status === "scheduled" || v.status === "arrived") ? <VisitMovementActions visitId={v.id} action={v.status === "scheduled" ? "arrive" : "depart"} /> : showVisits ? <Link className="pulse-inline-link" href="/portal/visits">Ouvrir →</Link> : null}</div>) : <p className="pulse-empty">Aucune visite aujourd’hui.</p>}</div></article>
      ...
      <article className="pulse-card" id="points-attention"><header><div><h2>! Points d’attention</h2><p>Accès direct aux écrans concernés</p></div></header><div>
        {late.map((p) => <div className="pulse-alert" key={`late-${p.id}`}><i className="danger" /><div><strong>Retour en retard · {nameOf(p.patient)}</strong><small>Retour prévu {tm(p.return_at)}</small></div><Link href="/portal/permissions">Traiter →</Link></div>)}
        {waiting.slice(0, 3).map((p) => <div className="pulse-alert" key={`waiting-${p.id}`}><i className="warning" /><div><strong>Permission à traiter · {nameOf(p.patient)}</strong><small>{dt(p.departure_at)}</small></div><Link href="/portal/permissions">Traiter →</Link></div>)}
        {highPriorityUnread.map((m) => <div className="pulse-alert" key={`message-${m.id}`}><i className={m.priority === 3 ? "danger" : "warning"} /><div><strong>Message niveau {m.priority} · {nameOf(m.sender)}</strong><small>{m.body.slice(0, 80)}</small></div><Link href="/portal/messages">Lire →</Link></div>)}
